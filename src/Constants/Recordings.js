// export const tapes = [
//   {
//     title:'Nie będzie powrotu',
//     year:1993,
//     label:'ABC Tapes',
//     medium:'Kaseta',
//     foto:'/images/niebedzie.jpg',
//     description:' Pierwszy niby-studyjny materiał, z tym, że studiem była klasa w mojej szkole, sprzętem - znaleziony przypadkiem w laboratorium elektronicznym mixer, a realizatorem Sexon - nasz kolega z osiedla, który w miarę ogarniał różnegi rodzaju sprzęty, a do tego miał w miarę dobry słuch.',
//     songs:[
//       '1. Intro', 
//       '2. Bunt 1',
//       '3. Beznadzieja (Bunt 2)',
//       '4. Beznadzieja (Bunt 4)',
//       '5. Miś',
//       '6. 2002',
//       '7. Antyskinheads',
//       '8. Narkomania',
//       '9. Kibol',
//       '10. Policyjne skurwysyny',
//       '11. Huta',
//       '12. To samo',
//       '13. Bez przyszłości',
//       '14. Nie będzie powrotu',
//       '15. Apokalipsa',
//       '16. Bilet do wojska',
//       '17. Przemoc',
//       '18. Wojna',
//       '19. Szkoła',
//       '20. Nazi Punk Fuck Off',
//     ]
//   },
//   {
//     title:'Kto zastrzelił dezertera?',
//     year:1994,
//     label:'Tylko Pank',
//     medium:'Kaseta',
//     foto:'/images/kto.jpg',
//     description:' Pierwszy materiał nagrany w studio - bodajże "Gamma Studio" dla wydawnictwa Tylko Pank',
//     songs:[
//       '1. Uciekająca wolność', 
//       '2. Obraz Twej nienawiści',
//       '3. Białą siła',
//       '4. Historia złotej bransoletni',
//       '5. Nie po to żyjemy',
//       '6. Generacja',
//       '7. Świat anrchistów',
//       '8. Krótka piosenka o jabłku',
//       '9. Słuchaj Jezu',
//       '10. Nie będzie powrotu instrumental',
//       '11. Fuck War',
//       '12. Nic o nas bez nas',
//       '13. Gdy będziesz odchodził',
//       '14. Szkoła',
//       '15. Reggae',
//       '16. Czas jest złudzeniem',
//       '17. Nowa wiara',
//       '18. Pasmo niepowodzeń',
//       '19. Ja to wiem',
//       '20. Huta',
//     ]
//   },
//   {
//     title:'Mijający czas',
//     year:1995,
//     label:'Tylko Pank',
//     medium:'Kaseta',
//     foto:'/images/mijajacy.jpg',
//     description:' Drugi materiał nagrany w studio - bodajże "Gamma Studio" dla wydawnictwa Tylko Pank',
//     songs:[
//       '1. Izba', 
//       '2. Pięść',
//       '3. 2002',
//       '4. Rasizm',
//       '5. Idziesz na wojnę',
//       '6. Jadą łysi (muz. Black Sabath)',
//       '7. Fałszywy',
//       '8. Ballada...(dla Zbycha)',
//       '9. Mijający czas',
//       '10. 1 maj',
//       '11. Ty',
//       '12. Mur nienawiści',
//       '13. Kto',
//       '14. Prywatki',
//       '15. Czemu jesteście',
//       '16. Czas rozstania',
//     ]
//   },
//   {
//     title:'Nie będziemy tacy',
//     year:2004,
//     label:'Jimmy Jazz Records',
//     medium:'CD',
//     foto:'/images/niebedziemy.jpg',
//     description:'Pierwszy materiał wydany na CD (Jimmy Jazz records, nagramy w Diamond Recordings Studio',
//     songs:[
//       '1. Urzędy', 
//       '2. Nie będziemy',
//       '3. Teraz niebo',
//       '4. To oni',
//       '5. Miliardy',
//       '6. Someone`s gonna die (muz. Blitz',
//       '7. Mój fiat',
//       '8. Nie zabijaj',
//       '9. Fucking life',
//       '10. Bez ostrzeżenia',
//       '11. Michał',
//       '12. Kolejne ofiary',
//       '13. Za pieniądze',
//       '14. Irak',
//       '15. Watch your back (muz.Cock Sparrer)',
//       '16. Dlaczego',
//       '17. AIDS',
//       '18. Instrumentalny',
//     ]
//   },
  

// ]

export const tapes = [
  // {
  //   title:'Nie będzie powrotu',
  //   year:1993,
  //   label:'ABC Tapes',
  //   medium:'Kaseta',
  //   foto:'/images/niebedzie.jpg',
  //   description:' Pierwszy niby-studyjny materiał, z tym, że studiem była klasa w mojej szkole, sprzętem - znaleziony przypadkiem w laboratorium elektronicznym mixer, a realizatorem Sexon - nasz kolega z osiedla, który w miarę ogarniał różnegi rodzaju sprzęty, a do tego miał w miarę dobry słuch.',
  //   songs:[
  //     '1. Intro', 
  //     '2. Bunt 1',
  //     '3. Beznadzieja (Bunt 2)',
  //     '4. Beznadzieja (Bunt 4)',
  //     '5. Miś',
  //     '6. 2002',
  //     '7. Antyskinheads',
  //     '8. Narkomania',
  //     '9. Kibol',
  //     '10. Policyjne skurwysyny',
  //     '11. Huta',
  //     '12. To samo',
  //     '13. Bez przyszłości',
  //     '14. Nie będzie powrotu',
  //     '15. Apokalipsa',
  //     '16. Bilet do wojska',
  //     '17. Przemoc',
  //     '18. Wojna',
  //     '19. Szkoła',
  //     '20. Nazi Punk Fuck Off',
  //   ]
  // },
  {
    title:'Kto zastrzelił dezertera?',
    year:1994,
    label:'Tylko Pank',
    medium:'Kaseta',
    foto:'/images/kto.jpg',
    description:' Pierwszy materiał nagrany w studio - bodajże "Gamma Studio" dla wydawnictwa Tylko Pank',
    songs:[
      {title:'1. Uciekająca wolność', 
        link:'/audio/Kto/01.mp3'},
      {title:'2. Obraz Twej nienawiści', 
        link:''},
      {title:'3. Biała siła', 
        link:''},
      {title:'4. Historia złotej bransoletki', 
        link:''},
      {title:'5. Nie po to żyjemy', 
        link:''},
      {title:'6. Generacja', 
        link:''},
      {title:'7. Świat anarchistów', 
        link:''},
      {title:'8. Krótka piosenka o jabłku', 
        link:''},
      {title:'9. Słuchaj Jezu', 
        link:''},
      {title:'10. Nie będzie powrotu instrumental', 
        link:''},
      {title:'11. Fuck war', 
        link:''},
      {title:'12. Nic o nas bez nas', 
        link:''},
      {title:'13. Gdy będziesz odchodził', 
        link:''},
      {title:'14. Szkoła', 
        link:''},
      {title:'15. Reggae', 
        link:''},
      {title:'16.Czas jest złudzeniem', 
        link:''},
      {title:'17. Nowa wiara', 
        link:''},
      {title:'18. Pasmo niepowodzeń', 
        link:''},
      {title:'19. Ja to wiem', 
        link:''},
      {title:'20. Huta', 
        link:''},
  
    ]
  },
  // {
  //   title:'Mijający czas',
  //   year:1995,
  //   label:'Tylko Pank',
  //   medium:'Kaseta',
  //   foto:'/images/mijajacy.jpg',
  //   description:' Drugi materiał nagrany w studio - bodajże "Gamma Studio" dla wydawnictwa Tylko Pank',
  //   songs:[
  //     '1. Izba', 
  //     '2. Pięść',
  //     '3. 2002',
  //     '4. Rasizm',
  //     '5. Idziesz na wojnę',
  //     '6. Jadą łysi (muz. Black Sabath)',
  //     '7. Fałszywy',
  //     '8. Ballada...(dla Zbycha)',
  //     '9. Mijający czas',
  //     '10. 1 maj',
  //     '11. Ty',
  //     '12. Mur nienawiści',
  //     '13. Kto',
  //     '14. Prywatki',
  //     '15. Czemu jesteście',
  //     '16. Czas rozstania',
  //   ]
  // },
  // {
  //   title:'Nie będziemy tacy',
  //   year:2004,
  //   label:'Jimmy Jazz Records',
  //   medium:'CD',
  //   foto:'/images/niebedziemy.jpg',
  //   description:'Pierwszy materiał wydany na CD (Jimmy Jazz records, nagramy w Diamond Recordings Studio',
  //   songs:[
  //     '1. Urzędy', 
  //     '2. Nie będziemy',
  //     '3. Teraz niebo',
  //     '4. To oni',
  //     '5. Miliardy',
  //     '6. Someone`s gonna die (muz. Blitz',
  //     '7. Mój fiat',
  //     '8. Nie zabijaj',
  //     '9. Fucking life',
  //     '10. Bez ostrzeżenia',
  //     '11. Michał',
  //     '12. Kolejne ofiary',
  //     '13. Za pieniądze',
  //     '14. Irak',
  //     '15. Watch your back (muz.Cock Sparrer)',
  //     '16. Dlaczego',
  //     '17. AIDS',
  //     '18. Instrumentalny',
  //   ]
  // },
  

]

